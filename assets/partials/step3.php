<div class="pop">
    <form id="appendSteps" class="form">
        <div id="step-3">
            ...
            <p class="amount"> <?php echo esc_html($price) ?> <?php echo esc_html(mb_strtoupper($currency)) ?></p>
            <img style="margin-left: auto; margin-right: auto" src="<?php echo esc_url($qrcode) ?>">

            <div id="countdown-timer" class="countdown">&nbsp;</div>

            <p>
                Only send <?php echo esc_html(mb_strtoupper($currency)) ?> to this address<br>
                <span class="address"><?php echo esc_html($address) ?></span>
            </p>

            <lottie-player src="<?php echo esc_url($siteUrl) ?>qr-code.json"  background="transparent"  speed="1"  style="width: 150px; height: 150px; margin: 0 auto;"  loop autoplay></lottie-player>
            <h3>Scan QR in your wallet</h3>
            <span>or</span>

            <a class="button button--full" target="_blank" href="<?php echo esc_html($url) ?>">Open in wallet</a>

            <div class="copyright">
                Powered by<br>
                <a href="https://ivendpay.com" target="_blank">
                    <img style="margin-left: auto; margin-right: auto" src="<?php echo esc_url($siteUrl) ?>ivendpay.png">
                </a>
            </div>
        </div>
    </form>
...
<template id="complete-message">
    <div class="bxc-complete-cnt bxc-box" style="display: block">
        <lottie-player src="<?php echo esc_url($siteUrl) ?>success.json"  background="transparent"  speed="1"  style="width: 150px; height: 150px; margin: 0 auto;" autoplay></lottie-player>
        <div class="bxc-title">Payment completed</div>
        <div class="bxc-text">Thank you for your payment</div>
        <a href="<?php echo esc_url($successOrder) ?>" class="button button--full">Back</a>
    </div>
</template>

<template id="timeout-message">
...
<script>
    (function( $ ) {
        ...
        setTimeout(function() {
          window.ivendPay.settings.site = '<?php echo esc_url($site) ?>';
          window.ivendPay.settings.ajaxUrl = '<?php echo esc_url($ajaxUrl) ?>';
          window.ivendPay.settings.price = '<?php echo esc_html($price) ?>';
          window.ivendPay.settings.currency_code = '<?php echo esc_html($currency) ?>';
          window.ivendPay.settings.hash = '<?php echo esc_html($hash) ?>';
          window.ivendPay.settings.formObject = $('.pop > .form');
          window.ivendPay.setToStorageData(<?php echo wp_kses_data(json_encode($data)) ?>);
          window.ivendPay.setIntervalTransaction();
        }, 1000);
    })( jQuery );
</script>
